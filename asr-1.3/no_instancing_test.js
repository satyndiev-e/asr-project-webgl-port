...
function main() {
    ...
    const geometry = asr.createGeometry(asr.geometryType().Triangles, triangle.rectangleVertices, triangle.rectangleIndices);
        ...
        for(let i = 0; i < ROWS; ++i) {
            for(let j = 0; j < COLS; ++j) {
                for(let k = 0; k < DEPTH; ++k) {
                    ...
                    asr.pushMatrix();
                    asr.translateMatrix(vec3.fromValues(x * scale, y * scale, z * scale));
