...
class Material {
    constructor() {
        ...
        this.textureTransformationEnabledUniformLocation = null;
        this.textureTransformationMatrixUniformLocation = null;
        this.textureNormalsEnabledUniformLocation = null;
...
function setMouseEventHandler(canvas) {
    let mouseX = 0;
...
function createMaterial(vertexShader, fragmentShader) {
    ...
    material.textureTransformationEnabledUniformLocation = gl.getUniformLocation(shaderProgram, "textureTransformationEnabled");
...
function createTexture(path, generateMipmaps = false) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, internalFormat, 1, 1, 0, internalFormat, gl.UNSIGNED_BYTE, new Uint8Array([0, 0, 255, 255]));
    ...
    image.onload = () => {
        ...
        if(isPowerOf2(image.width) && isPowerOf2(image.height)) {
            ...
            const ext = gl.getExtension("EXT_texture_filter_anistropic");
            if(ext) {
                gl.texParameterf(gl.TEXTURE_2D, ext.TEXTURE_MAX_ANISTROPY_EXT, 0.0);
            }
        }
    }
    image.onerror = () => {
    ...
    texture.transformMatrix = transformMatrix;
...
function renderCurrentGeometry() {
    ...
    try {
        ...
        let textureTransformationEnabled = currentTexture !== null && currentMaterial.textureTransformationEnabledUniformLocation !== null;
        if (currentMaterial.textureTransformationEnabledUniformLocation !== null) {
            gl.uniform1i(currentMaterial.textureTransformationEnabledUniformLocation, textureTransformationEnabled ? 1 : 0);
        }
